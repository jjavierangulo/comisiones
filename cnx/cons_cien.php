...
function TraerConsulta($Agente,$FechaInicio,$FechaFin, $Cliente){
	...
	$VarClientes = " and ( ";
	foreach ($Cliente as $Nombrecliente) {

		$Or = ($VarClientes == " and ( ") ? " " : " or ";

		$NombreclienteAux = ($Nombrecliente == "HOTELERÍA") ? "HOTELERIA" : $Nombrecliente;
    	$VarClientes.= $Or.  "  IPCLIENTE.TipoCliente COLLATE SQL_Latin1_General_Cp1_CI_AI  like '%$NombreclienteAux%' "; //"". $Nombrecliente . "-";
  	} 
  	$VarClientes.= " )" ;
	...
	$msQuery = "SELECT /* ANTICIPO */ 'A' as TIPO, PAGO.[DocNum] as 'RBO',replace(convert(NVARCHAR, T0.[DocDate], 106), ' ', '/') as 'FRECIBO',PAGO.U_AGENTE 'AGTE', 
				...
				case 
				when ROUND(SUMAARTICULOS.DESCTO,2) <= .07 then '12%'
				when ROUND(SUMAARTICULOS.DESCTO,2) <= .12 then '10%'
				else '8%'
				end as 'COMISION',
				case 
					when ROUND(SUMAARTICULOS.DESCTO,2) <= .07 then ROUND((PAGO.COBRANZASINIVA - SUMAARTICULOS.INSTALACION - SUMAARTICULOS.VIATICOS) * .12,2)
					when ROUND(SUMAARTICULOS.DESCTO,2) <= .12 then ROUND((PAGO.COBRANZASINIVA - SUMAARTICULOS.INSTALACION - SUMAARTICULOS.VIATICOS) * .10,2)
					else ROUND((PAGO.COBRANZASINIVA - SUMAARTICULOS.INSTALACION - SUMAARTICULOS.VIATICOS) * .08,2)
				end as 'COMISIONTOTAL',
				CASE 
				...
				and  PAGO.[InvType] =203 and T0.[Canceled]='N'  AND T0.TrsfrAcct>'11102000' AND T0.TrsfrAcct<'11200000' $VarClientes
			union /* COBRANZA */
